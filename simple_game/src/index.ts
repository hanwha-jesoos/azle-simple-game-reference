...
export default Canister({
  ...
  lotteryBattle: update([text], Battle, async (battleId) => {
    /*
        게임 진행 및 정산
      */
    const battle = _getBattleByUuid(battleId);
    ...
    // 3. 참여자의 금액을 기준으로 추첨확률을 계산하여 1 명의 당점자를 선택한다.
    // 각 참여자들의 result를 battle.results에 추가한다.

    let participantsList: { owner: any; allowance: any; probability: any;}[] = [];
    for (let character of battle.characters) {
      const allowance = await _allowanceFrom(character.owner);
      ...
      if (allowance < battle.betAmount) {
        ...
        // 참여자의 금액을 기준으로 확률을 계산합니다.
        participantsList.forEach((participant) => {
          participant.probability = participant.allowance / participantsList.reduce((sum, participant) => sum + participant.allowance, 0);
        });
    ...
    // 당첨자를 무작위로 선택합니다.
    let list = participantsList.filter((participant) => participant.probability > 0);
